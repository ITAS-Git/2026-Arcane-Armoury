...
socket.on("state_updated", (incoming) => {
  ...
  saveState(s);
  render(s);
  // DM inputs should reflect incoming state too
  if (typeof writeInputsFromState === "function") writeInputsFromState(s);
});
...
socket.on("hp_delta", ({ player, delta }) => {
  ...
  render(normalized);
  // Only DM re-broadcasts to sync everyone
  if (IS_DM) socket.emit("state_set", normalized);
});
